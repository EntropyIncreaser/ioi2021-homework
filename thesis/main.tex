...
\begin{solution}
首先根据符号化方法\footnote{Symbolic Method，见 \cite[A.I]{acomb}}的理论，设节点的生成函数为 $A(x) = \sum_k a_kx^k$，可以立即列出生成函数方程 $T(x) = A(x) \exp \left(\frac{T(x^k)}{k}\right)$。
...
如果无需求出所有系数，藉由多元 Lagrange 反演，我们可以给出 $\tilde O(2^n)$ 的求算方法。
